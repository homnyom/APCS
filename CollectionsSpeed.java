 //  name: Natalie Homnyom      date: 1/11/16    
import java.util.*;
...
public class CollectionsSpeed
{
   ...
   public static void makeValues(ArrayList<Integer> al, LinkedList<Integer> ll)
   {
      for(int x = 0; x < N; x++)
      {
         al.add((int) (Math.random()*N));
         ll.add((int) (Math.random()*N));
      }
   }
   ...
   public static double timeAddLast(List<Integer> list)
   {
      long start = System.nanoTime();
      for(int x = 0; x < N; x++)
         list.add(list.size(), (int) (Math.random()*N));
      return (System.nanoTime() - start)/1E6;
   }
   ...
   public static double timeAddLastLL(List<Integer> list)
   {
      ...
      for(int x = 0; x < N ; x++)
        newlist.addLast((int) (Math.random()*N));
      return (System.nanoTime() - start)/1E6;
   }
}
